...
{
    sealed class GlulxVocabFormat : IVocabFormat
    {
        readonly Context ctx;

        int nextPreposition = 255;    // fits in a byte for syntax lines
        int nextAdjective = 255, nextBuzzword = 255, nextVerb = 255;

        public GlulxVocabFormat(Context ctx)
        {
            this.ctx = ctx;
        }

        public IWord CreateWord(ZilAtom text) => new GlulxParserWord(text);

        public void MergeWords(IWord dest, IWord src)
        {
            ((GlulxParserWord)dest).Merge(ctx, (GlulxParserWord)src);
        }

        void MakePart(IWord word, ISourceLine location, PartOfSpeech part, string description,
            Func<GlulxParserWord, Action<Context, ISourceLine, int>> getSetter, ref int next, bool onlyNumberedInV3 = false)
        {
            var result = (GlulxParserWord)word;
        ...
        {
            ((GlulxParserWord)result).SetObject(location);
        }
        ...
        {
            ((GlulxParserWord)word).WriteToBuilder(ctx, wb, helpers.DirIndexToPropertyOperand);
        }

        static bool CheckPart(IWord word, PartOfSpeech part) => (((GlulxParserWord)word).PartOfSpeech & part) != 0;
        ...
        public bool IsSynonym(IWord word) => ((GlulxParserWord)word).SynonymTypes != PartOfSpeech.None;
        ...
        static int GetPart(IWord word, PartOfSpeech part) => ((GlulxParserWord)word).GetValue(part);
