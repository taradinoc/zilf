...
namespace Zilf
{
    class Compiler
    {
                ...
                foreach (Syntax line in verb)
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if (!match)
                                Errors.CompWarning(cc.Context, null,
            ...
            try
            {
                ...
                switch (head.StdAtom)
                {
                    ...
                    case StdAtom.DEC:
                        ...
                        if (atom == null)
                        {
                            Errors.CompError(cc.Context, form, "expected an atom after INC/DEC");
                            return wantResult ? cc.Game.Zero : null;
                        }
                        if (cc.Locals.TryGetValue(atom, out local))
                            variable = local;
                        ...
                        {
                            rb.EmitBinary(
                                head.StdAtom == StdAtom.INC ? BinaryOp.Add : BinaryOp.Sub,
                                variable,
                                cc.Game.One,
                                variable);
                            return variable;
        ...
        private static bool HasSideEffects(CompileCtx cc, ZilObject expr)
        {
            ...
            // TODO: move metadata like this into attributes on the atoms?
            switch (head.StdAtom)
            {
                case StdAtom.AGAIN:
                case StdAtom.APPLY:
        ...
        private static bool HasResult(TernaryOp op)
        {
            // currently no TernaryOps have a result
            return false;
        }

        private static IOperand CompileNullaryOp(CompileCtx cc, IRoutineBuilder rb, NullaryOp op,
            ZilList args, bool wantResult, IVariable resultStorage)
        {
            if (args.First != null)
                throw new CompilerError("nullary operation must have no args");

            bool hasResult = HasResult(op);
            IVariable result = !hasResult ? null : (resultStorage ?? rb.Stack);

            rb.EmitNullary(op, result);

            if (wantResult && !hasResult)
                return cc.Game.One;
            else
                return result;
        }

        private static IOperand CompileUnaryOp(CompileCtx cc, IRoutineBuilder rb, UnaryOp op,
            ZilList args, bool wantResult, IVariable resultStorage)
        {
            if (args.First == null || args.Rest.First != null)
                throw new CompilerError("unary operation must have exactly 1 arg");

            bool hasResult = HasResult(op);

            // special case for RANDOM, which may be used in void context
            bool flush = false;
            if (hasResult && !wantResult)
            {
                if (op == UnaryOp.Random)
                    ...
                    throw new CompilerError("non-void unary operation in void context");
            }

            IVariable result = !hasResult ? null : (resultStorage ?? rb.Stack);

            IOperand value = CompileAsOperand(cc, rb, args.First);
            rb.EmitUnary(op, value, result);
        ...
        private static IOperand CompileBinaryOp(CompileCtx cc, IRoutineBuilder rb, BinaryOp op,
            ZilList args, bool wantResult, IVariable resultStorage)
        {
            if (args.First == null || args.Rest.First == null || args.Rest.Rest.First != null)
                throw new CompilerError("binary operation must have exactly 2 args");

            bool hasResult = HasResult(op);

            if (hasResult && !wantResult)
                throw new CompilerError("non-void binary operation in void context");

            IVariable result = !hasResult ? null : (resultStorage ?? rb.Stack);

            using (Operands operands = Operands.Compile(cc, rb, args.First, args.Rest.First))
            {
                rb.EmitBinary(op, operands[0], operands[1], result);
            }
        ...
        private static IOperand CompileTernaryOp(CompileCtx cc, IRoutineBuilder rb, TernaryOp op,
            ZilList args, bool wantResult, IVariable resultStorage)
        {
            if (args.First == null || args.Rest.First == null || args.Rest.Rest.First == null ||
                args.Rest.Rest.Rest.First != null)
                throw new CompilerError("ternary operation must have exactly 3 args");

            bool hasResult = HasResult(op);

            if (hasResult && !wantResult)
                throw new CompilerError("non-void ternary operation in void context");

            IVariable result = !hasResult ? null : (resultStorage ?? rb.Stack);

            using (Operands operands = Operands.Compile(cc, rb,
                args.First, args.Rest.First, args.Rest.Rest.First))
            {
                rb.EmitTernary(op, operands[0], operands[1], operands[2], result);
            }
